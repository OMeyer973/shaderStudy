		...
		SubShader
	{
		...
		Pass
		{
			...
			float3 rotateY(float3 v, float rad) {
				float cosY = cos(rad);
				float sinY = sin(rad);
				//float3x3 rotMat(cosY, 0, sinY, 0, 0, 0, -sinY, 0, cosY);
				//return p * rotMat;
			...
			float4 distanceField(float3 p)
			{
				...
				float4 spheres = float4(_sphereColors[0], sdSphere(p - _sphere.xyz, _sphere.w));
				for (int i = 1; i < 8; i++)
				{
					float4 newSphere = float4(_sphereColors[i], sdSphere(rotateY(p, _angularOffset * i) - _sphere.xyz, _sphere.w));

					spheres = opUS(newSphere, spheres, _smoothFactor);
				}

				return opUS(ground, spheres, _smoothFactor);
			}
